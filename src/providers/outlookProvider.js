...
class OutlookProvider extends BaseEmailProvider {
  ...
  async connect() {
    ...
    try {
      ...
      this.logger.info('Outlook provider connected');
    } catch (error) {
      this.logger.error(`Outlook connection failed: ${error.message}`);
      const err = new Error(`Outlook authentication failed: ${error.message}`);
  ...
  async disconnect() {
    if (this.transporter) {
      ...
      this.logger.debug('Outlook transporter closed');
    }
  }
}
